...
bfi_merged_sf <- bfi_geom |>
  ...
  filter(!is.na(predicted_GCP_current_USD))

#### 5. Data Processing for Plotting ####

bfi_merged_sf$scaled_gdp <- bfi_merged_sf$predicted_GCP_current_USD * 1e9 
...
plot_title_EN <- "Bolivia GDP (2021) by 0.25°x 0.25° cells"
...
ggsave("bolivia_gdp_map_2.png", plot = bolivia_map, width = 8, height = 6, dpi = 300)
...
municipality_sf_bolivia_agg <- bolivia_municipalities |>
...
plot_title_municipality_EN <- "Bolivia GDP per capita (2021) by municipality"
...
bolivia_municipality_map <- ggplot() +
  geom_sf(
...
print(bolivia_municipality_map)
